...
def build_design_matrix(embeddings, durations, n_components=30, device="cpu", return_shuffles=False, n_shuffles=100):
    ...
    pcs = PCA(n_components=n_components).fit_transform(np.vstack(embeddings))
    ...
    return X_scaled_torch, X_raw, scaler, Xs_list, prepare_features
...
def build_XY_continuous(X_meta, Y_meta, spike_data, region, target_speaker="SPK1",
    ...
    X_scaled, X_raw, scaler, Xs_list, prepare_features = build_design_matrix(
