...
Follow-up variant targeting the 7 patients that came back "not significant" in the
pc60/2.5:1-cap replicate run (YFK, YFG, YFF, YFM, YFP, YFU, YEZ): PC=30, and STRICT
1:1 balancing (both sides downsampled to exactly the smaller side's count) instead of
the 2.5:1 cap, to check whether the null result holds under tighter balance/lower PCs.

- Window: self = onset-200ms, len 500ms; all other speakers = onset+200ms, len 500ms
  (spike tag fixed_selfm200_otherp200_len500).
- Balancing: MAX_RATIO=1.0 -- whenever n_a != n_b, the larger side is randomly
  subsampled (seed=42) down to exactly match the smaller side's count.
...
PATIENTS = {  # restricted below to the 'not significant' subset via sys.argv
...
def fit_pair(patient, X_pca, masks, Ys, spkA, spkB):
    ...
    out_path = os.path.join(RESULTS_DIR, f"threeway_rcross_strictbal_{patient}_{pair_tag}_bert_L12_pc30.pkl")
...
def main():
    ...
    out_csv = os.path.join(RESULTS_DIR, "threeway_rcross_strictbal_bert_L12_pc30_summary.csv")
