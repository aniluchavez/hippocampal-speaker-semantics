...
Follow-up to three_way_r_cross_pc30_strictbalance.py: that script balanced each of
the 3 pairs independently, so a given speaker's kept word count differed pair to
pair (e.g. self=1202 in the 1/2 pair but self=420 in the 1/3 pair for the same
patient). This version balances ONCE per patient -- all three speakers (self,
otherA, otherB) are downsampled (seed=42) to a single shared N = min of their three
original counts -- so 1/2, 1/3, and 2/3 all use identical word counts for every
role, making the three r_cross values directly comparable within a patient.

- Window: self = onset-200ms, len 500ms; all other speakers = onset+200ms, len 500ms
  (spike tag fixed_selfm200_otherp200_len500).
- Balancing: global per-patient cap to N = min(n_self, n_otherA, n_otherB), applied
  once to all three speakers before building any of the three pairs.
...
def fit_pair(patient, X_pca, masks_bal, Ys_bal, spkA, spkB):
    ...
    out_path = os.path.join(RESULTS_DIR, f"threeway_rcross_globalbal_spktag_truewindow_{patient}_{pair_tag}_bert_L12_pc100.pkl")
...
def run_patient(patient: str) -> list[dict]:
    ...
    for spk in ("Speaker1", spkA, spkB):
        ...
        if Y is None or Y.shape[0] != mask.sum():
            ...
            return []
        # Drop individual word-rows with NaN spike counts (e.g. window extends before
        # recording start) rather than letting one bad row zero out every neuron via
        # the downstream sum() filter.
    ...
    # Global per-patient balancing: one shared N across all three speakers.
    ...
    for pA, pB in [("Speaker1", spkA), ("Speaker1", spkB), (spkA, spkB)]:
        row = fit_pair(patient, X_pca, masks_bal, Ys_bal, pA, pB)
        if row is not None:
            rows.append(row)
    return rows
...
def main():
    ...
    out_csv = os.path.join(RESULTS_DIR, "threeway_rcross_globalbal_spktag_truewindow_bert_L12_pc100_summary.csv")
