...
def _maybe_gaussian_smooth(spike_train, sigma_ms=None, preserve_sum=True):
    """Optionally smooth a 1D spike train with a Gaussian kernel.

    Assumptions:
      - spike_train is sampled at 1 ms resolution (so sigma is in ms == samples).
      - intended for FIRING-RATE estimation (do NOT use for spike *counts*).

    Parameters
    ----------
    sigma_ms : float | None
        Standard deviation of the Gaussian in milliseconds (samples).
        If None or <= 0, no smoothing is applied.
    preserve_sum : bool
        If True, rescales the smoothed signal so total sum matches the original.
    """
    if sigma_ms is None:
        return spike_train
    try:
    ...
    orig_sum = np.nansum(x)

    y = gaussian_filter1d(x, sigma=sigma, mode="nearest")

    if preserve_sum:
        new_sum = np.nansum(y)
        if np.isfinite(orig_sum) and np.isfinite(new_sum) and new_sum > 0:
            y = y * (orig_sum / new_sum)
    return y
...
def process_and_save_spikes(spikes, region_cells, speaker_events, speaker_windows=None, binSize=20, output_dir="output", mode="fixed_speaker_windows", gaussian_sigma_ms=None, gaussian_preserve_sum=True):
    """
        ...
        if mode == "fixed_speaker_windows":
            ...
            for region, neuron_indices in list(region_cells.items()):
                ...
                for neuron_idx in range(n_neurons):
                    ...
                    neuron_spike_train = _maybe_gaussian_smooth(neuron_spike_train, sigma_ms=gaussian_sigma_ms, preserve_sum=gaussian_preserve_sum)
                    neuron_event_bins = []
...
def compute_spike_features(
    ...
    feature_mode="counts",   # "counts" | "rates_mean" | "rates_binned"
    binSize=20,              # ms, used for rates_binned
    gaussian_sigma_ms=None,  # ms, optional (applied to continuous 1ms train before binning)
    gaussian_preserve_sum=True,
    flatten_time=True,
    fixed_n_bins=None,
    pad_value=0.0,
):
    """Compute spike features per event per neuron.

    Parameters
    ----------
    spikes : array [T x N_total]
        1 ms sampled spike train (0/1 or counts per ms).
    region_cells : dict {region: neuron_indices}
    event_times : array [n_events x K]
        In explicit_event_bounds mode:
          col 1 = pre_onset (ms index)
          col 2 = post_offset (ms index)
          col 3 = duration (ms)  (optional but expected in your pipeline)
    feature_mode : str
        - "counts": raw spike sums per window (words x neurons)
        - "rates_mean": mean firing rate per window in Hz (words x neurons)
        - "rates_binned": time-resolved binned firing rates in Hz.
            Output shape:
              - if flatten_time=True: (words x (neurons * n_bins))
              - else: (words x n_bins x neurons)

    Notes
    -----
    - Gaussian smoothing only materially affects "rates_binned" (time-resolved features).
      For "rates_mean", mass-preserving smoothing yields ~the same window-average.
    """
    import numpy as np
    ...
    word_pres = event_times[:, 1]
    word_posts = event_times[:, 2]
    # duration column is optional; if missing, derive from bounds
    if event_times.shape[1] >= 4:
        durs_ms = event_times[:, 3].astype(float)
    else:
        durs_ms = (word_posts - word_pres + 1).astype(float)
    ...
    for region, neuron_indices in list(region_cells.items()):
        ...
        if feature_mode in ("counts", "rates_mean"):
            # compute spike sums first
            region_result = np.full((num_events, n_neurons), np.nan, dtype=float)

            for neuron_idx in range(n_neurons):
                train = region_spikes[:, neuron_idx]
                ...
                for i in range(num_events):
                    ...
                    if ce <= cs:
                        continue
                    region_result[i, neuron_idx] = np.sum(train[cs:ce])

            if feature_mode == "rates_mean":
                secs = np.where(durs_ms > 0, durs_ms / 1000.0, np.nan)[:, None]
                region_result = region_result / secs

            result_by_region[region] = region_result
            continue

        if feature_mode == "rates_binned":
                # time-resolved: (optional) smooth -> cut window -> bin -> Hz
                # IMPORTANT: decoding needs fixed feature width across self/other, so we enforce fixed_n_bins.
                bin_ms = int(binSize)
                if bin_ms <= 0:
                    raise ValueError("binSize must be > 0 for rates_binned")

                if fixed_n_bins is None:
                    raise ValueError(
                        "rates_binned requires fixed_n_bins (e.g., window_length_ms // binSize_ms) "
                        "to keep feature dimensions consistent across conditions."
                    )
                n_bins = int(fixed_n_bins)
                if n_bins <= 0:
                    raise ValueError("fixed_n_bins must be > 0")

                # Allocate [events x bins x neurons] (pad_value=0.0 is sklearn-friendly)
                R = np.full((num_events, n_bins, n_neurons), float(pad_value), dtype=float)

                for neuron_idx in range(n_neurons):
                    train = region_spikes[:, neuron_idx].astype(float)

                    # optional smoothing on the full continuous train
                    train = _maybe_gaussian_smooth(
                        train,
                        sigma_ms=gaussian_sigma_ms,
                        preserve_sum=gaussian_preserve_sum
                    )
                    ...
                    for i in range(num_events):
                        ...
                        seg = train[cs:ce]
                        nb_avail = len(seg) // bin_ms
                        nb = min(nb_avail, n_bins)
                        if nb <= 0:
                            continue

                        seg = seg[: nb * bin_ms]
                        seg_bins = seg.reshape(nb, bin_ms).sum(axis=1)
                        seg_rates = seg_bins / (bin_ms / 1000.0)  # Hz

                        R[i, :nb, neuron_idx] = seg_rates

                if flatten_time:
                    # (events x (neurons*bins)) in neuron-major order
                    result_by_region[region] = R.transpose(0, 2, 1).reshape(num_events, n_neurons * n_bins)
                else:
                    result_by_region[region] = R
