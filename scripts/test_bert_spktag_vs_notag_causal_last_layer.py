#!/usr/bin/env python3
"""Apples-to-apples test: causal BERT last-layer (L12) hippocampal encoding,
WITH vs WITHOUT speaker tags in the input tokenization. Attention masking
(causal) is held constant across both arms, unlike the earlier bidirectional-
no-tag vs causal-spktag comparison.

Paired across patients (patient-level median held-out pseudo-R^2), separately
for self/other, same fixed-window pc50 fit (self: -300 to +200 ms; other:
+20 to +520 ms).
"""
