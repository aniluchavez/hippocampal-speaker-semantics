"""
Square-1, AR-control + circular-shift variant — SHUFFLE-CV counterpart to
square1_ar_xcirc.py. Identical in every respect (AR neural-history control,
circularly-shifted embeddings as the null) except CV folds are random KFold
instead of contiguous blocks.

Rationale for testing this alongside the block-CV version: block CV doesn't
just remove autocorrelation leakage, it also forces the model to generalize
across topic boundaries it may never have seen in training -- a different,
harder question than "does word-level semantic content predict spiking."
Under shuffle CV, paired with a null that preserves X's own autocorrelation
(circular shift, not full xshuffle), both the real fit and the null get the
SAME train/test temporal-adjacency advantage -- so the comparison stays fair
even though the CV folds ignore topic structure. If this and the block-CV
version agree, the leakage concern was specifically about null-mismatch
(xshuffle), not CV-mode itself. If they disagree, block CV may be unfairly
conservative due to topic-shift generalization gaps.

Usage: python3 -u scripts/square1_ar_xcirc_shufflecv.py
"""
...
# ── DATA LOADING (same as square1_ar_control.py / square1_ar_xcirc.py) ───────
...
def block_splits(n, k):
    """Random KFold -- shuffle CV (per user's generalization-across-topics
    concern with contiguous blocks). Paired with circular-shift null below so
    the real fit and the null both get the same temporal-adjacency advantage."""
...
for cond, mask in [("self", mask_self), ("other", mask_other)]:
    ...
    print(f"  sig unique_sem|AR (xcirc-shift, shuffle CV): {sig.sum()}/{n_m} ({100*sig.mean():.1f}%)  "
...
print("\n" + "="*70)
print("SUMMARY  (semantic vs AR-history control, shifted-X circular null, shuffle CV)")
print("="*70)
summary_df = pd.DataFrame(results_summary)
print(summary_df.to_string(index=False))
out_path = "/scratch/aniluchavez/ConvoDATAS/VPResults/square1_ar_xcirc_shufflecv_PTYEU_hippo.pkl"
