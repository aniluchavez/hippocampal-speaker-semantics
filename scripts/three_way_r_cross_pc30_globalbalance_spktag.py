...
def fit_pair(patient, X_pca, masks_bal, Ys_bal, spkA, spkB):
    ...
    out_path = os.path.join(RESULTS_DIR, f"threeway_rcross_globalbal_spktag_{patient}_{pair_tag}_bert_L12_pc30.pkl")
...
def run_patient(patient: str) -> list[dict]:
    ...
    for spk in ("Speaker1", spkA, spkB):
        ...
        if Ys[spk] is None or Ys[spk].shape[0] != masks[spk].sum():
...
def main():
    ...
    out_csv = os.path.join(RESULTS_DIR, "threeway_rcross_globalbal_spktag_bert_L12_pc30_summary.csv")
