...
with unique semantic encoding: p_perm < 0.05 AND unique_semantic > 0 in either
self or other condition, using worddur VP (BERT-base L12, pc30, xcirc + block CV + sym_perm).

Output: figures/11_brackets_bert_worddur_pc30_sigVP_L12.pdf
"""
...
VP_DIR  = VPR_DIR / "bert-base_ctx200_worddur_xcirc_symperm" / "pc30"
REL_DIR = GLM_DIR / ("bert-base_ctx200_bestfixed_selfm300_len500_otherp20_len500"
                     "_notebookexact_shuf_xcirc_r2only") / "pc50"

# ── load VP significance data ─────────────────────────────────────────────────
df_vp = pickle.load(open(VP_DIR / "L12_VP_all.pkl", "rb"))
...
# ── significance filter: p_perm<0.05 & unique_semantic>0 in self OR other ─────
hippo_vp = df_vp[df_vp["region"] == "hippocampus"].copy()

sig_self = set(zip(
    hippo_vp.loc[(hippo_vp["condition"] == "self") & (hippo_vp["p_perm"] < 0.05) & (hippo_vp["unique_semantic"] > 0), "patient"],
    hippo_vp.loc[(hippo_vp["condition"] == "self") & (hippo_vp["p_perm"] < 0.05) & (hippo_vp["unique_semantic"] > 0), "neuron_idx"],
))
sig_other = set(zip(
    hippo_vp.loc[(hippo_vp["condition"] == "other") & (hippo_vp["p_perm"] < 0.05) & (hippo_vp["unique_semantic"] > 0), "patient"],
    hippo_vp.loc[(hippo_vp["condition"] == "other") & (hippo_vp["p_perm"] < 0.05) & (hippo_vp["unique_semantic"] > 0), "neuron_idx"],
))
...
print(f"Sig either: {len(sig_either)}")
print(f"r_cross rows: {len(df_sig)}")
...
ax.set_title(
    f"Hippocampus — unique semantic neurons\n"
    f"VP: BERT-base L12, worddur pc30, xcirc+block+sym_perm\n"
    f"Reliability: bestfixed window, BERT-base L12\n"
    f"n={n_neurons} neurons, {n_patients} patients  |  p_perm<0.05 & unique_sem>0",
    fontsize=10, fontweight="bold",
)
ax.spines[["top", "right"]].set_visible(False)

out_pdf = FIG_DIR / "11_brackets_bert_worddur_pc30_sigVP_L12.pdf"
out_png = FIG_DIR / "11_brackets_bert_worddur_pc30_sigVP_L12.png"
out_svg = FIG_DIR / "11_brackets_bert_worddur_pc30_sigVP_L12.svg"
