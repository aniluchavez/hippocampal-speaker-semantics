...
(BERT L12, PC=60, fixed_selfm200_otherp100_len500), so directly comparable to
the pc30 and pc200 runs. PCA is fit once per patient and reused for all 3 pairs.
"""
import os
import sys
import pickle
...
def fit_pair(patient, X_pca, masks, Ys, spkA, spkB):
    ...
    out_path = os.path.join(RESULTS_DIR, f"threeway_rcross_{patient}_{pair_tag}_bert_L12_pc60.pkl")
...
def main():
    ...
    out_csv = os.path.join(RESULTS_DIR, "threeway_rcross_bert_L12_pc60_summary.csv")
