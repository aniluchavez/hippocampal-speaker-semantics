#!/usr/bin/env python3
"""SVG model/layer semantic encoding plot with solid model lines, no grid, no fastText."""
...
OUT = Path(
    "/scratch/aniluchavez/ConvoDATAS/SemanticGLM/plots/"
    "all15_bestfixed_model_layer_performance_solid_nogrid_no_fasttext.svg"
)
...
def main() -> int:
    ...
    df = df.loc[~df["model"].eq("fasttext-wiki")].copy()
    ...
    fig, axes = plt.subplots(1, 2, figsize=(10.4, 4.55), sharey=True)
    fig.subplots_adjust(left=0.085, right=0.81, bottom=0.16, top=0.80, wspace=0.12)
    ...
    for ax, condition, title in zip(
        ...
        for model, label, color, marker in MODELS:
            ...
            ax.plot(x, y, color=color, linewidth=2.1, linestyle="-", alpha=0.94, zorder=2)
            ax.scatter(
                x,
                y,
                s=34,
