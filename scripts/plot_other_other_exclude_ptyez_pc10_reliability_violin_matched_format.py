...
BERT bidirectional-max-context embeddings, PC10, patient PTYEZ excluded.
...
Source data: SemanticGLM/other_other_rcross_reliability_bert_pc10/
             other_other_exclude_ptyez_reliability_violin_longdata.csv

Output: figures/other_other_exclude_ptyez_pc10_reliability_violin_matched_format.pdf
"""
...
DATA_DIR = Path("/scratch/aniluchavez/ConvoDATAS/SemanticGLM/other_other_rcross_reliability_bert_pc10")
FIG_DIR  = PROJECT / "figures"
FIG_DIR.mkdir(exist_ok=True)

# ── load long-format data, pivot to wide ───────────────────────────────────────
long_raw = pd.read_csv(DATA_DIR / "other_other_exclude_ptyez_reliability_violin_longdata.csv")
wide_raw = long_raw.pivot_table(index=["patient", "pair", "neuron"],
                                 columns="metric", values="value").reset_index()

RENAME = {
    "Speaker A\nsplit-half": "External speaker A",
    "Speaker B\nsplit-half": "External speaker B",
    "Cross-speaker\nr":      "Cross-speaker",
    "Noise\nceiling":        "noise_ceiling",
}
wide_raw = wide_raw.rename(columns=RENAME)
wide_raw["lower_ceiling"] = wide_raw[["External speaker A", "External speaker B"]].min(axis=1)
wide_raw["r_cross"] = wide_raw["Cross-speaker"]
...
ax.set_title(
    "Hippocampal beta reliability between external speakers, BERT bidirectional max context, PC10, PTYEZ excluded",
    fontsize=6.5, fontweight="bold",
)
ax.spines[["top", "right"]].set_visible(False)

out_pdf = FIG_DIR / "13_other_other_exclude_ptyez_pc10_reliability_violin_matched_format.pdf"
out_png = FIG_DIR / "13_other_other_exclude_ptyez_pc10_reliability_violin_matched_format.png"
out_svg = FIG_DIR / "13_other_other_exclude_ptyez_pc10_reliability_violin_matched_format.svg"
fig.savefig(out_pdf)
